...
{
	Mat img_blurred;
	Size ksize = Size(5, 5);	// must be odd (5 is recommended)
	double sigma = 2.0;		// must be in [1.0, 2.0]
...
int main()
{
	...
	imwrite("stage3.jpeg", img_suppressed);
	...
	imwrite("stage4_strong.jpeg", img_strong);
	imwrite("stage4_weak.jpeg", img_weak);

	// stage 5 - hysteresis
	// input: img_weak, img_strong
	// output: img_edges
	Mat img_edges = hysteresis(img_weak, img_strong);
	imwrite("stage5.jpeg", img_edges);
